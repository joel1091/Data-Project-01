...
# URL del archivo JSON (ajustar según corresponda)
JSON_URL = "https://valencia.opendatasoft.com/api/explore/v2.1/catalog/datasets/fgv-estacions-estaciones/exports/json?lang=es&timezone=Europe%2FBerlin"
...
def main():
    try:
        ...
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS public."fgv_estaciones" (
            gid INTEGER,
            "Codi / Código" INTEGER,
            "Nom / Nombre" VARCHAR(255),
            "Tipus / Tipo" INTEGER,
            "Línia / Línea" VARCHAR(255),
            "Pròximes Arribades / Próximas Llegadas" VARCHAR(255),
            proximas_llegadas VARCHAR(255),
            geo_shape GEOMETRY,
        ...
        for item in data:
            ...
            proximas_arribades = item.get("proximas_arribades")
            ...
            cursor.execute("""
            INSERT INTO public."fgv_estaciones" 
            (gid, "Codi / Código", "Nom / Nombre", "Tipus / Tipo", 
            "Línia / Línea", "Pròximes Arribades / Próximas Llegadas", 
            proximas_llegadas, geo_shape, geo_point_2d) 
            VALUES (%s, %s, %s, %s, %s, %s, %s, ST_GeomFromGeoJSON(%s), ST_GeogFromText(%s))
            """, (gid, codigo, nombre, tipo, linea, proximas_arribades, proximas_llegadas, geojson_str, point_wkt))
            print(f"Insertado registro con gid={gid}, nombre={nombre}")
